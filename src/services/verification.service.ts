...
class VerificationService {
  ...
  async updateStatus(id: string, status: VerificationStatus): Promise<Verification> {
    const verification = await db.verification.update({ where: { id }, data: { status } });
    if (status === "VERIFIED") {
      await db.user.update({ where: { id: id }, data: { isVerified: true } })
    }
    if (status === "REJECTED") {
      await db.user.update({ where: { id: id }, data: { isVerified: false } })
    }
    if (status === "PENDING") {
      await db.user.update({ where: { id: id }, data: { isVerified: false } })
    }
